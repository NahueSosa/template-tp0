...
public class RegExGenerator {
    ...
    private String generateResult(List<Field> fields){
        ...
        for (Field field: fields) {
            int times = 0;
            switch (field.quant){
                case QUANT_ASTERISK: times = getRandomIntBetween(0, maxLength); break;
                case QUANT_PLUS: times = getRandomIntBetween(1, maxLength); break;
                case QUANT_QUEST: times = getRandomIntBetween(0, 1); break;
                case QUANT_ONE: times = 1; break;
            }
            ...
            while (count < times) {
                switch (field.chars.charAt(0)) {
                    case BACKSLASH:
                        partialResult.append(field.chars.substring(1, 2)); break;
                    case OPEN_BRACKET:
                        partialResult.append(getRandomCharFromSet(field.chars.substring(1, field.chars.length() - 1))); break;
                    case ANY_CHAR:
                        partialResult.append(getRandomCharacter()); break;
                    default:
                        partialResult.append(field.chars.substring(0, 1)); break;
                }
                count++;
            }
        }
    ...
    private Field getFieldFromChar(String subRegEx){
        ...
        switch (subRegEx.charAt(1)){
            case QUANT_ASTERISK: return new Field(subRegEx.substring(0, 1), QUANT_ASTERISK);
            case QUANT_PLUS: return new Field(subRegEx.substring(0, 1), QUANT_PLUS);
            case QUANT_QUEST: return new Field(subRegEx.substring(0, 1), QUANT_QUEST);
            default: return new Field(subRegEx, QUANT_ONE);
        }
    }
